...
router.get('/:scrapeId/graph', roles.can('access public app'), function(req, res, next) {
	...
	if (req.params.scrapeId == 'mostrecent') {
		...
		getGraphByScrapeId(req.params.scrapeId)
			.then(function(graph){
				res.json(graph)
			})
	}
...
var getGraphByScrapeId = function(scrapeId) {
	return models.Friendship.getAllByScrapeId(scrapeId, models)
		.then(function(friendships) {
